...
fn create_best_practices_test_project() -> tempfile::TempDir {
    use std::fs;

    let dir = tempfile::tempdir().expect("temp dir");
    let vector_dir = dir.path().join(".vector");
    let prompt_dir = dir.path().join("doc").join("prompts").join("best-practices");
    ...
    fs::write(
        vector_dir.join("language-rules.yaml"),
        "rust:\n  best-practices: prompts-00006-rust\n",
    )
    .expect("write language config");

    fs::write(
        prompt_dir.join("prompts-00006-rust.md"),
        "---\nid: prompts-00006-rust\ntype: prompts\ncode: \"00006\"\nslug: rust\n---\n\n# Rust Best Practices\nFollow idiomatic Rust patterns.\n",
    )
    .expect("write rust best practices prompt");
...
/// Verifies that `BestPracticesOp` can be selected and dispatched through
/// the standard `PluginDispatcher` path without any MCP types involved.
#[tokio::test]
async fn best_practices_op_dispatches_through_dispatcher() {
    let dir = create_best_practices_test_project();
    let root = IoPath::new(dir.path());

    let (_cancel, mut receiver) = PluginDispatcher::new(BestPracticesOp::new())
        .input(BestPracticesInput::new(root, vec!["rust".to_string()]))
        .build()
        .expect("dispatcher build must succeed with valid input");
    ...
    assert!(
        output.prompt.contains("# Rust Best Practices"),
        "output prompt must contain the resolved body"
    );
}

/// Verifies that `LanguageBestPracticesParams` deserializes with the required fields.
#[test]
fn language_best_practices_params_deserialize_correctly() {
    let raw = r#"{"root_dir": "/tmp/project", "languages": ["rust", "typescript"]}"#;
    let params: super::LanguageBestPracticesParams =
        serde_json::from_str(raw).expect("must deserialize LanguageBestPracticesParams");
    assert_eq!(params.root_dir, "/tmp/project");
    assert_eq!(params.languages, vec!["rust", "typescript"]);
}
...
async fn language_best_practices_tool_executes_best_practices_path() {
    ...
    let tools = super::LanguageTools::new();
    let result = tools
        .language_best_practices(Parameters(super::LanguageBestPracticesParams {
            root_dir: dir.path().display().to_string(),
            languages: vec!["rust".to_string()],
        }))
        .await
    ...
    assert!(
        result.contains("# Rust Best Practices"),
        "tool result must contain the resolved prompt body"
    );
    assert!(
        !result.contains("id: prompts-00006-rust"),
        "tool result must strip frontmatter before returning the prompt"
    );
}

/// Verifies that the MCP-facing tool skips unconfigured languages and succeeds
/// when at least one requested language has a best-practices mapping.
#[tokio::test]
async fn language_best_practices_tool_skips_unconfigured_languages() {
    let dir = create_best_practices_test_project();
